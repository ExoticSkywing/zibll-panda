...
function zibpay_mail_payment_order_to_admin($values)
{
    ...
    //发送微信模板消息（带错误保护 - Xingxy Patch）
    $type = 'payment_order_admin'; // 使用变量代替字面量，解决引用传参问题
    try {
        $wechat_template_id = @zib_get_wechat_template_id($type);
    } catch (Exception $e) {
        $wechat_template_id = false;
...
function zibpay_mail_payment_order_to_referrer($values)
{
    ...
    //发送微信模板消息（带错误保护 - Xingxy Patch）
    $type = 'payment_order_to_referrer';
    try {
        $wechat_template_id = @zib_get_wechat_template_id($type);
    } catch (Exception $e) {
        $wechat_template_id = false;
